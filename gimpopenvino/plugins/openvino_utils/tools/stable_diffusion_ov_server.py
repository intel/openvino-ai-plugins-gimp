...
def run(model_name, available_devices, power_mode):
    ...
    if model_name not in fast_sd_models_up:
        model_paths = {
			...
			"sd_1.5_square_fp8": ["stable-diffusion-ov", "stable-diffusion-1.5", "square_fp8"],
        ...
        engine = initialize_engine(fast_sd_models_map[model_name], model_name, available_devices)
...
def handle_client_data(data, conn, engine, model_name, model_path, scheduler):
    ...
    if data.decode() == "model_name":
        ...
        conn.sendall(tosend)
        return
    if model_name in fast_sd_models_up:
        try:
            weight_path = get_weight_path()
            option_cache_file = os.path.join(weight_path, "..", "gimp_openvino_run_fastsd.json")
            ...
            log.info('Starting FastSD inference... ')
            ...
            height = options.get("src_height")
            width = options.get("src_width")
            output = engine(
                    prompt=prompt,
