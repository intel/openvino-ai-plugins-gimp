...
class StableDiffusionThreeEngine(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.register_modules(
            ...
            text_encoder_2 = self.load_model(os.path.join(model, "text_encoder_2"), device[0], ov_config),
            text_encoder_3 = None,
            tokenizer = AutoTokenizer.from_pretrained(os.path.join(model,"tokenizer")),
            tokenizer_2 = AutoTokenizer.from_pretrained(os.path.join(model,"tokenizer_2")),
            tokenizer_3 = None,
        ...
        self._text_encoder_3_dim = 4096


    def load_model(self, model_name, device, config=None):
        if "NPU" in device:
            with open(os.path.join(f"{model_name}.blob"), "rb") as f:
                return self.core.import_model(f.read(), device)
        return self.core.compile_model(os.path.join(f"{model_name}.xml"), device, config if "text_encoder" in model_name else {})
...
def init_pipeline(models_dict: Dict[str, Any], device: str, use_flash_lora: bool, text_encoder_3_dim=4096):
    ...
    ov_pipe = StableDiffusionThreeEngine(**pipeline_args)
    return ov_pipe
