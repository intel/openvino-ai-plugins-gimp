...
class AsyncPipeline:
    def __init__(self, ie, model, model_path, plugin_config, device='CPU', max_num_requests=1):
        ...
        ie.set_property({'CACHE_DIR': os.path.join(model_path, "..", 'cache')})

        self.logger.info('Model Cached')        

        self.logger.info('Loading network to {} plugin...'.format(device))
        
        self.exec_net = ie.compile_model(model_path, device, plugin_config)
        self.async_queue = AsyncInferQueue(self.exec_net, max_num_requests)

        if max_num_requests == 0:
            # ExecutableNetwork doesn't allow creation of additional InferRequests. Reload ExecutableNetwork
            # +1 to use it as a buffer of the pipeline
            self.async_queue  = AsyncInferQueue(self.exec_net, len(self.async_queue) + 1)

        self.empty_requests = deque(self.async_queue)
