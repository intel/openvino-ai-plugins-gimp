...
class oldLatentConsistencyEngine(DiffusionPipeline):
    def __init__(
            self,
            # scheduler: Union[DDIMScheduler, PNDMScheduler, LMSDiscreteScheduler],
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                model_pred = \
                self.unet([latents, ts, prompt_embeds, w_embedding], share_inputs=True, share_outputs=True)[0]
        ...
        print("type(engine image) = ", type(image[0]))
...
class LatentConsistencyEngine(DiffusionPipeline):
    ...
    def __call__(
        ...
        print("Ready to return: image is ", image)
        return image[0]
