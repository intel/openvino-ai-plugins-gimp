...
def styletransfer(procedure, image, drawable, device_name, model_name, progress_bar, config_path_output):
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        GimpUi.init("fast-style-transfer-ov.py")
...
class Styletransfer(Gimp.PlugIn):
    ## Parameters ##
    __gproperties__ = {
        "model_name": (
            str,
            _("Model Name"),
            "Model Name: 'mosaic', 'candy', 'pointilism', 'udnie','rain-princess' ",
            "mosaic",
    ...
    def do_create_procedure(self, name):
        procedure = None
        if name == "fast-style-transfer-ov":
