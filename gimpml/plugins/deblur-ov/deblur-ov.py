...
def deblur(procedure, image, drawable, force_cpu, gpu, vpu, progress_bar, config_path_output):
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        python_path = config_path_output["python_path"]
        config_path_output["plugin_path"] = os.path.join(config_path, "deblur-ov.py")
        ...
        GimpUi.init("deblur-ov.py")
        use_header_bar = Gtk.Settings.get_default().get_property(
            "gtk-dialogs-use-header"
        )
        dialog = GimpUi.Dialog(use_header_bar=use_header_bar, title=_("OV Deblur..."))
...
class Deblur(Gimp.PlugIn):
    ...
    def do_create_procedure(self, name):
        ...
        if name == "deblur-ov":
            ...
            procedure.set_menu_label(N_("OV Deblur..."))
